...
## Разовое создание структуры товаров из xlsx (Масла.xlsx)

Структура на Яндекс.Диске: **Товары** / **Группа** / **Название товара** / типы: Кросс коды, Фото, Видео, Этикетки, Документы. В папке «Кросс коды» создаются пустые файлы по артикулам.

1. Запустите приложение (`npm run dev` или после деплоя откройте сайт).
2. Вызовите API с файлом xlsx (колонки: **Группа**, **Названия**, **Артикул**, **Объём**):

```bash
curl -X POST -F "file=@Масла.xlsx" "http://localhost:3000/api/yandex/seed-from-xlsx"
```

(Замените URL на ваш домен при деплое.) В ответе будет список созданных путей и возможные ошибки.
